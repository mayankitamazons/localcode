...
<script type="text/javascript">
  ...
  $( document ).ready(function() {
	...
	$(document).on("click",'.without_varient', function(event) { 
		...
		var input="<input type='hidden' name='name[]' id='name' value='"+p_name+"'/><input type='hidden' name='subpro_id[]' id='subpro_id' value=''/>";
		// alert(input);
		var o="<tr class='orderitem'><td id="+id+" class='text-center' onclick='deleterow(this)'><i class='fa fa-times tip pointer posdel' title='Remove' style='cursor:pointer;'></i></td><td>"+p_name+"</td><td>"+p_price+"</td><td>"+extra_price+"</td><td><input style='width:50px;' type=number name='qty[]' maxlength='3' class='product_qty qd' onchange='changeqty(this)'  value="+quantity+" id='"+id+"_test_athy'><input type='hidden' name='p_id[]' value= "+id+">";
		o+=input+"<input type='hidden' name='p_code[]' value= "+code+"><input type='hidden' name='ingredients' value='" + single_remarks + "'/></td>";
		o +="<td><input type='text' class='form-control subt' style='width:90px' id='subtotal"+id+"' value='"+p_total+"' name='subtotal[]' readonly=''></td>";
		o +="<td> <a href='#remarks_area' role='button' class='introduce-remarks btn btn-large btn-primary' data-toggle='modal'>" + ((single_remarks == '') ? "Remarks" : single_remarks) +  "</a></td>";
		o +="</tr>";
