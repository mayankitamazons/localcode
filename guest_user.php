...
{	
  $vcount=0;
   ...
   //if($stl_key == $_SESSION['stl_key']) {
        ...
        $sql = "INSERT INTO order_list_temp SET varient_type='$v_str',product_id='$pro_id',  session_id='$session_id', user_id='', merchant_id='$m_id', quantity='$qty_list', amount='$p_price',product_code='$p_code', remark='$option', location='".$location."', table_type='".$table_type."',section_type='$section_type',created_on='$date', invoice_no='$invoice_no',invoice_seq='$invoice_seq'";
	  ...
	  if($varient_type)
			{ 
				...
				foreach($varient_type as $v)
				{
					...
					foreach($varr as $single)
					{
						$product_id=$parray[$vs];
						if($single)
						{
							$v_id=$single;
							  mysqli_query($conn, "INSERT INTO order_varient SET product_id='$product_id', varient_id='$v_id', invoice_no='$invoice_no',order_id='$order_id',merchant_id='$m_id'");
						}
					} 
					$vs++;
				}
   ...
   if(isset($_POST['guest_login']))
   {
      ...
      $already_exists11 = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM users WHERE mobile_number='$cm' && user_roles= '1'"));
      
      if($already_exists11 > 0){
          $error .= "you are already member.Please login  .<br>";
      }
      ...
      if($check_user != 0){
         ...
         //  header("location:login.php"); 
             ...
             $error .= "10 times free order is over. Please register .<br>";
         }
            ...
            if($error == ""){
                ...
                $user_id = $get_user['id'];
				$user_mobile = $get_user['mobile_number'];
            
                 $upt_tt = mysqli_query($conn, "UPDATE `order_list_temp` SET user_id='$user_id',user_mobile='$user_mobile' WHERE id = '$o_id' ");
				// echo $user_id;
				// echo $o_id;
				// die;
         ...
         if($error == ""){
            ...
            if($guest_permission == "0"){
                    mysqli_query($conn, "INSERT INTO users SET name='$name',user_roles='$user_role',mobile_number='$cm'");
            
              // $user_total = mysqli_fetch_assoc(mysqli_query($conn, "SELECT MAX(id) as id FROM `users` WHERE user_roles = '3'"));
               // $user_id=$user_total['id'];
			   $user_id=mysqli_insert_id($conn);
                $o_id = $_SESSION['order_id'];
                $upt_tt = mysqli_query($conn,"UPDATE `order_list_temp` SET user_id='$user_id',user_mobile='$cm' WHERE id = '$o_id' ");
               $url="location:order_place.php?user_id=".$user_id."&order_id=".$o_id;
               header($url); 
            } else {
              $_SESSION['order_id'] = $o_id;
              $url="location: login_guest.php";
              header($url);
            }
     ...
     <?php 
                     ...
                     <label>Mobile Number <span style="color:red;">*</span></label>
                              <div class="login-top">
                                 ...
                                 <input type="text" id="mobile_number" class="mobile_number" placeholder="012-3115670" name="mobile_number"  />
                                 <!--br>
                                 <label>User Name</label>
                                 <input type="text" class="name active" placeholder="User Name" name="name" value="" id="reg_name">
                                 <br>
                                 <br>
                                  <div class="select_optionss">
                                        <input type="radio" name="user_role" value="3" checked> Guest user
                           </div!-->
                         
						 
                                 <div class="login-bottom login-bottom1">
                                    <div class="submit">
                              <input type="hidden" value="<?php echo $order_total['id']; ?>" name="o_id" />
                              <?php if($guest_permission == "0"){?>
							  <br/>
                                        <input type="submit" value="Confirm" class="btn btn-primary" name="guest_login" style="font-size: 20px;"/>
                                        <?php } else {?>
                                        <button class="btn btn-primary" type="button" data-toggle="modal" data-target="#myModal">Confirm</button>
                                        <?php }?>
                                    </div>
                                 </div>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <form id ="data">
                                                <div class="modal-body" style="padding-bottom:0px;">
                                                    Due to misuse of order system by unknown user, we request you to register as a member in order to complete the order. Kindly login or register.
                                                </div>
												<?php if($merchant_id!='670'){ ?>
                                                <div class="modal-footer" style="padding-bottom:10px;">
												<a class="" href="<?php echo $site_url;?>/facebook-login/fbconfig.php"><img src="img/login-cont-facebook.jpg" style="min-height:45px"></a>
                                                    <button class="btn btn-primary" type="submit" name="member_login">Login/Register</button>
                                                </div>
												<?php } ?>
                                            </form>
                                        </div>
                                ...
                                <div class="login-bottom login-bottom1">
                                    <label style="font-size: 24px;">Or</label>
                                </div>
								<?php if($merchant_id!='670'){ ?>
								 <div class="login-bottom login-bottom1" style="width:100%;clear:both; padding-top: 20px;">
<div class="Back">
<a class="" href="<?php echo $site_url;?>/facebook-login/fbconfig.php"><img src="img/login-cont-facebook.jpg" style=""></a>
</div>
</div>  
								<?php } ?>
                                 <div class="login-bottom login-bottom1">
                                    <div class="Back">
                                        <a class="col-md-2" href="login_guest.php?invitation_id=<?php echo $_SESSION['invitation_id'];?>"><img src="img/join-us.jpg" style="width: 100px;"></a>
                                    </div>
                                 </div>
