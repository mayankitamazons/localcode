...
error_reporting(E_ALL);
    ...
    while ($row=mysqli_fetch_assoc($data)){
		...
		// die;    
        $item = array('remark_extra'=>$remark_extra,'membership_discount'=>$membership_discount,'order_extra_charge'=>$order_extra_charge,'order_place'=>$order_place,'wallet_paid_amount'=>$wallet_paid_amount,'final_amount'=>$final_amount,'varient_type'=>$varient_type,'register' => $register, 'sst' => $sst, 'gst' => $gst, 'user_id' => $user_id, 'product_code' => $product_code, 'table_type' => $table_type,'section_type'=>$section_type, 'location' => $location, 'remark' => $remark_ids, 'invoice_no' => $row['invoice_no'] , 'status' => $row['status'] , 'id' => $row['id'] , 'username' =>$order_name, 'merchantname' => $merchant_name, 'product_name' => $array_product_names, 'product_qty' => $product_qty, 'product_amt' => $product_amt);
        array_push($array_detail, $item);
    }
    echo json_encode($array_detail);
}
...
if( isset( $_POST['method']) && ( $_POST['method'] == "neworder" )  ) {
 ...
 if( isset( $_POST['method']) && ( $_POST['method'] == "usercheck" )  ) {
  ...
  if( isset( $_POST['method']) && ( $_POST['method'] == "normalorder" ) ) {
    ...
    $ref_result = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id='".$_SESSION['login']."'"));
...
if( isset( $_POST['method']) && ( $_POST['method'] == "getUnPrintedOrder2" )  ) {
	...
	if( $ref_result['order_print_setting'] === 'on' ) {
        ...
        $merchant_name = $ref_result['name'];
		  ...
		  {
			$data = mysqli_query($conn, "SELECT * FROM order_list WHERE status=0 AND merchant_id='$id' AND auto_print!='1' and  date(created_on)='$date' and created_timestamp >= NOW() - INTERVAL 10 MINUTE ORDER BY ID DESC LIMIT 10");
        
		  }
		  else
		  {
			$data = mysqli_query($conn, "SELECT * FROM order_list WHERE status=0 AND merchant_id='$id' and  date(created_on)='$date' and order_place='local' and auto_print!='1' and  created_timestamp >= NOW() - INTERVAL 10 MINUTE ORDER BY ID DESC LIMIT 10");
...
if( isset( $_POST['method']) && ( $_POST['method'] == "pintOrderold" ) ) {
	$print='n';
	$order = $_POST['order'];
	$product_ids=$order['product_ids'];
	$profile_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id='".$_SESSION['login']."'"));
...
function singleorderprint($ip_address,$data,$date,$time,$order,$conn,$s_t,$s_name)
{
		...
		else
		{
			...
			try {
				...
				if($order['remark_extra'])
				$printer -> text("  Order Remark : " . $order['remark_extra']."\n");
	   ...
	   // die;
	   	if( $order == null ) {
			...
			try {
				...
				$cash_pay=$total-$wallet_paid_amount-$membership_discount;
				...
				{
					$membership_discount=number_format($membership_discount,2);
					$printer -> text( "         "."Member  Discount:        RM $membership_discount" . "   " . "\n");
				}
			    if($cash_pay>0)
				{
	   ...
	   // die;
	   	if( $order == null ) {
			...
			try {
				...
				$cash_pay=$total-$wallet_paid_amount-$membership_discount-$discount_amount;
				...
				{
					$membership_discount=number_format($membership_discount,2);
					$printer -> text( "         "."Membership Discount:           RM $membership_discount" . "   " . "\n");
				}
				if($discount_amount>0)
				{
					$discount_amount=number_format($discount_amount,2);
	...
	{     
	   	...
	   	if( $order == null ) {
			...
			try {
				...
				// $printer -> text("\n");
				$printer -> selectPrintMode(Printer::MODE_FONT_A);
				$printer -> text( "Name( Code )    Qty  Remark  Price  Amount " . "\n");
				// $printer -> text("\n");
				$total = 0;
				$qty_total = 0;
				$v_order=1;
				...
				// for( $i = 0 ; $i < sizeof( $sorder ) ; $i ++ ) {
					...
					for( $j = 0 ; $j < $lines ; $j++) {  
						...
						$amount_print = str_pad($amount_print,  $size_amount, "   ");
						$printer -> textChinese( ' ' .  $name_print . '    ' . $qty_print. ' ' . $remark_print . ' ' . $price_print . ' ' . $amount_print . "\n");
