...
<script src="./Dashboard_files/isotop.min.js"></script>
  ...
  <script type="text/javascript">
			...
			$(document).ready(function(){
				...
				$('.qtyInput').numpad({displayTpl: '<input class="form-control" type="number" />'});
                            ...
                            <div id="left-bottom">
                                <table id="totalTable"
                                   ...
                                   <!--   <textarea name="payment_note[]" id="payment_note_2" class="pa form-control kb-text" aria-haspopup="true" role="textbox"></textarea>-->
                                           <input class="form-control kb-text qtyInput" value="<?php if(isset($_GET['t'])){ echo $_GET['t'];} ?>"  id="table" name="table" type="text" />
                                          </div>
                                        </td>
                                      </tr>
              ...
              <div class="modal-content">
                  ...
                  <div class="modal-footer" style="position: relative;padding:40px 0 25px 0">
                    <div style="position: absolute;left:5px;text-align: left;top:5px">
                        Quantity: <input type="number" class="qtyInput" name="quantity_input" value="1" style="width:2em;text-align:center;"/>
                    </div>
  ...
  $( document ).ready(function() {
   ...
   $("#remarks_area").on("shown.bs.modal", function(){
    ...
    $('#remarks_area').on('click','.save_close', function (e) {
        ...
        $(".introduce-remarks.selected").parent().parent().find("input[name='subpro_price[]']").val(input_extras);
        $(".introduce-remarks.selected").parent().parent().find("input[name='subtotal[]']").val((input_extras + unitPrice < 0) ? 0 : (input_extras + unitPrice).toFixed(2));
   ...
   // alert(rmk_val);
    ...
    $.ajax({
      type: 'post',
      url: 'productData.php',
      data: {p_id:id,sp_id:ex_id,remark:remark,extravar:extravar,rmk_val:rmk_val,extra_value:extra_value,varient_type:varient_type},
      success: function (data) {
      //alert(data);
      if( data != null ) {
			//alert('The product added');
   ...
   else
   {
    ...
    $("#varient_name").html(name);
      ...
      $(".pop_model").html("<div class='row' style='width:11em'><div class='col-md-12'>Quantity: <input name='quantity_input' type='number' class='quatity qtyInput' value='1' style='width:2em;text-align:center' min='0' max='99'></div></div><input type='hidden' name='single_ingredients' value='" + single_remarks + "'/><input type='hidden' name='extra' value='" + p_extra + "'/><span id='pop_cart' data-pr=" + p_price + " data-id='"+id+"' data-code='"+code+"'  data-name='"+name+"' data-quantity='"+quantity+"'>Add to Cart</span>");
