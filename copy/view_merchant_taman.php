...
<body onload="initialize()" class="header-light sidebar-dark sidebar-expand pace-done">
    ...
    <?php } ?>
        ...
        <?php } else { ?>
        <form id="order_place" action="order_place.php" method="post">
        <?php } ?>
  ...
  <div style="float:left;width:100%;top:0;">
    ...
    <div style="float:left;width:33%;">
        ...
        <input type="hidden" name='varient_count' value='0' id='varient_count'/>
            ...
            <input type="Number" class="form-control table"  name="table_type"  <?php if($merchant_detail['table_required']=="1"){ echo "required";} ?>  value="<?php echo $tablenumber; ?>"/>
...
<?php } ?>
   ...
   <div class=" modal fade" id="AlredyShow" role="dialog">
        <div class="element-item modal-dialog modal-dialog-centered" style="position: absolute;top: 0;bottom: 0;left: 0;right: 0;display: grid;align-content: center;">
            ...
            <div class="element-item modal-content">
                <div class="element-item modal-header">
                                    ...
                                    <p>Same order already placed in last two minute,Please place again</p>
                   <div id="" class="modal-footer model_pop" style="padding-bottom:2px;">
                  
				  <a href="view_merchant.php"> <button role="button" class="btn btn-large btn-primary" style="background:#50D2B7;border:none;">Ok</button></a>
                  </div>    
                                </div>
                            </div>
    </div>
   ...
   if(go_ahead=="y")
   {
    ...
    var p_extra = $(this).siblings("input[name='extra']").val();
     ...
     $(".element-item input[name='extra']").val('');
      ...
      // alert(varient_type);
      $("#test").append("<tr>  <td><button type='button' class='removebutton'>X</button> </td><td>"+name+"</td><td><input style='width:50px;'  onchange='UpdateTotal("+id+" ,"+p_price+")'  type=number name='qty[]' maxlength='3' class='product_qty'  value="+quantity+" id='"+id+"_test_athy'><input type= hidden name='p_id[]' value= "+id+"><input type= hidden name='p_code[]' value= "+code+"><input type='hidden' name='ingredients' value='" + single_remarks + "'/></td><td><a href='#remarks_area' role='button' class='introduce-remarks btn btn-large btn-primary' data-toggle='modal'>" + ((single_remarks == '') ? "Remarks" : single_remarks) +  "</a><input type='hidden' name='extra' value='"+extra_price+"'></td><td>"+code+"</td><td><input style='width:70px;text-align:right;' type='text' name='p_extra' value='" + extra_price.toFixed(2) + "' readonly></td><td><input style='width:70px;' type='text' name='p_price[]' value='"+p_price+"' readonly></td><td><input type='text' style='width:70px;' name='p_total[]' value= "+ p_total + " readonly  id='"+id+"_cat_total'><input type='hidden' name='varient_type[]' value="+varient_type+"></td> </tr>");
      // alert('The product added');
      $(".quatity").val(1);
...
$(document).ready(function(){
  ...
  var al_exit='<?php echo $al_exit; ?>';
  var shop_status=$('#shop_status').val();
 if(al_exit=="y")
 {
	$("#AlredyShow").modal("show");
 }
