...
/* end  for limit  */
  $query="SELECT order_list.*, sections.name as section_name FROM order_list left join sections on order_list.section_type = sections.id WHERE merchant_id ='".$_SESSION['login']."' ORDER BY order_list.created_on DESC LIMIT $offset, $rec_limit";
// die;
if(isset($_GET['q']) && isset($_GET['cr'])){
    ...
    // $query="SELECT order_list.*, users.mobile_number FROM order_list inner join users on order_list.user_id = users.id WHERE (merchant_id ='".$_SESSION['login']."' OR order_list.main_merchant='" . $merchant_name['name'] . "') AND $field LIKE '%$get_query%' ORDER BY `created_on` DESC LIMIT $offset, $rec_limit";
       $query="SELECT order_list.*, sections.name as section_name FROM order_list left join sections on order_list.section_type = sections.id WHERE (merchant_id ='".$_SESSION['login']."') AND $field LIKE '%$get_query%' ORDER BY order_list.created_on DESC LIMIT $offset, $rec_limit";
}else{
     $query="SELECT order_list.*, sections.name as section_name FROM order_list left join sections on order_list.section_type = sections.id WHERE merchant_id ='".$_SESSION['login']."' ORDER BY order_list.created_on DESC LIMIT $offset, $rec_limit";
}
$total_rows = mysqli_query($conn,$query);
$total_rows1 = mysqli_query($conn,$query);
 $last_id = mysqli_fetch_assoc(mysqli_query($conn, "SELECT max(id) as max_id FROM order_list WHERE merchant_id ='".$_SESSION['login']."'"))['max_id'];
            ...
            <div class="row" id="main-content" style="padding-top:25px">
              ...
              <form id="criteria_form">
                <div class="row">
                  ...
                  <div class="col-sm-2">
                    <input type="text" class="form-control" name="q" placeholder="Enter your criteria ..." value="<?php echo (isset($_GET['q'])) ? $_GET['q'] : ''; ?>">
                  </div>
                  <div class="col-sm-1">
				      					...
				      					<div class="credentials-container">
											...
											<div>
											  <div class="input-group mb-2" style="margin-bottom:0px !important;">
												...
												<input type="hidden"  value="<?php echo $_SESSION['login']; ?>" id="wallet_merchant_id"/>
...
//end code
    ...
    $(document).ready(function(){
        ...
        e.preventDefault();
        var criteria = $(this).parent().parent().find("select[name='cr']").find("option:selected").attr("val");
        var query = $(this).parent().parent().find("input[name='q']").val();
		...
		$("#confirm_payment").click(function() {
			...
			if(mobile_num)
			{
				$.ajax({
					  type: "POST",
					  url: "functions.php",
					  data: {mobile_num:mobile_num,method:"deductfund",merchant_id:wallet_merchant_id,paid_wallet_amount:paid_wallet_amount},
					  cache: false,
					  success: function(data) {
                            ...
                            if( data != null ) {
                                ...
                                if (flag == 1) {
                                  ...
                                  for( var i = 0 ; i < order['product_name'].length ; i ++ ) {
                                    ...
                                    total_amount += total;
                                    total = total.toFixed(2);
									$("#paid_wallet_amount").val(total_amount);
