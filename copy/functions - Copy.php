...
if( isset( $_POST['method']) && ( $_POST['method'] == "pintOrder" ) ) {
	...
	foreach($product_ids as $p)
	{
		...
		$product_result[] = mysqli_fetch_array(mysqli_query($conn, "SELECT products.id,products.print_ip_address,products.product_name,products.product_type,products.remark,products.printer_ip_2 FROM products WHERE products.id in($p)"));
	 ...
	 // $prow=mysqli_fetch_array($product_result);
	  ...
	  // die;
		...
		foreach($arr as $kp=>$p)
		{
			...
			$print_report=singleorderprint($kp,$p,$date,$time,$order,$conn);
			// break;
		}
		// print_R($arr2);
		// die;   
...
function singleorderprint($ip_address,$data,$date,$time,$order,$conn)
{
	// echo $ip_address;
		...
		else
		{
			...
			try {
				...
				foreach($data as $d)
				{
					...
					// print_R($pr_str);
					// die;
					$product_id=$pr_str['product_id'];
								$name=$pr_str['product_name'];
								$qty=$pr_str['product_qty'];
								$remark=$pr_str['remark'];
								$p_code=$pr_str['product_type']; 
								$product_remark=$pr_str['p_remark'];
					$number = $i + 1;
					$j=0;
					...
					$v_data =mysqli_query($conn, "SELECT varient_id FROM order_varient WHERE product_id='$product_id' and invoice_no='$invoce_id'");
