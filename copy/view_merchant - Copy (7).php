  ...
  <div style="float:left;width:100%;top:0;">
   ...
   <div class="container">
    <div class="row">
        <div class="col-xs-6">
		<?php if($online_pay){ ?>
           <input type="submit" style="width:100% !important;" class="btn btn-block btn-primary submit_button" name="cashpayment"  id="confm"  value="Confirm (E-wallet/ Credit Card)">
   ...
   if(go_ahead=="y")
   {
    ...
    var p_extra = $(this).siblings("input[name='extra']").val();
     ...
     $(".element-item input[name='extra']").val('');
      ...
      // alert(varient_type);
      $("#test").append("<tr>  <td><button type='button' class='removebutton'>X</button> </td><td>"+name+"</td><td><input style='width:50px;'  onchange='UpdateTotal("+id+" ,"+product_price+")'  type=number name='qty[]' maxlength='3' class='product_qty'  value="+quantity+" id='"+id+"_test_athy'><input type= hidden name='p_id[]' value= "+id+"><input type= hidden name='p_code[]' value= "+code+"><input type='hidden' name='ingredients' value='" + single_remarks + "'/></td><td>"+code+"</td><td><a href='#remarks_area' role='button' class='introduce-remarks btn btn-large btn-primary' data-toggle='modal'>" + ((single_remarks == '') ? "Remarks" : single_remarks) +  "</a><input type='hidden' name='extra' value='"+extra_price+"'></td><td><input style='width:70px;text-align:right;' type='text' name='p_extra' value='" + extra_price.toFixed(2) + "' readonly></td><td><input style='width:70px;' type='text' name='p_price[]' value='"+product_price+"' readonly></td><td><input type='text' style='width:70px;' name='p_total[]' value= "+ p_total + " readonly  id='"+id+"_cat_total'><input type='hidden' name='varient_type[]' value="+varient_type+"></td> </tr>");
      // alert('The product added');
      $("#varient_type").html('');   
      $("#varient").html('');  
   }
