...
if( isset( $_POST['method']) && ( $_POST['method'] == "forgotpass" )  ) {
 ...
 if( isset( $_POST['method']) && ( $_POST['method'] == "startcash" )  ) {
	  ...
	  if($cash_id)
		$query="update cash_system set opening='$opening_balance' where is_active='y'";
...
if( isset( $_POST['method']) && ( $_POST['method'] == "getUnPrintedOrder" )  ) {
    ...
    $ref_result = mysqli_fetch_assoc(mysqli_query($conn, "SELECT name, id, order_print_setting, sst, gst, register,company FROM users WHERE id='".$id."'"));
    if( $ref_result['order_print_setting'] === 'on' ) {
        $sst = $ref_result['sst'];
        $gst = $ref_result['gst'];  
        $register = $ref_result['register'];
...
if( isset( $_POST['method']) && ( $_POST['method'] == "normalorderuser" ) ) {
    ...
    $date = $_POST['date'];
    $time = $_POST['time'];
	 $ref_result = mysqli_fetch_assoc(mysqli_query($conn, "SELECT id, print_ip_address,printer_style,printer_profile,usb_name,company FROM users WHERE id='".$merchant_id."'"));
	// print_R($ref_result);
	// die;
    $ref_result2 = mysqli_fetch_assoc(mysqli_query($conn, "SELECT id, print_ip_address_user FROM users WHERE id='".$_SESSION['login']."'"));
	// print_R($ref_result);
...
if( isset( $_POST['method']) && ( $_POST['method'] == "normalorder" ) ) {
    ...
    $ref_result = mysqli_fetch_assoc(mysqli_query($conn, "SELECT merchant_url,id,sst_rate,customer_print_ip_address,print_ip_address,printer_style,printer_profile,usb_name,company FROM users WHERE id='".$_SESSION['login']."'"));
	// print_R($ref_result);
	// die;
	if($ref_result['printer_profile']=="usb")
	{
	}
	else
	{
...
if( isset( $_POST['method']) && ( $_POST['method'] == "pintOrder" ) ) {
    ...
    $ref_result = mysqli_fetch_assoc(mysqli_query($conn, "SELECT merchant_url,id,sst_rate,print_ip_address,printer_style,printer_profile,usb_name,company FROM users WHERE id='".$_SESSION['login']."'"));
		 ...
		 // die;
	   	if( $order == null ) {
			...
			// print_R($merchat_detail);
			 ...
			 $company=$merchat_detail['company'];
