...
/* end  for limit  */
$query="SELECT order_list.*, users.mobile_number FROM order_list inner join users on order_list.user_id = users.id WHERE merchant_id ='".$_SESSION['login']."' ORDER BY `created_on` DESC LIMIT $offset, $rec_limit";
...
while ($row=mysqli_fetch_assoc($total_rows1)){
    ...
    if((intval($diff_total_minute) > intval($pending_time)) && ($row['status'] == 0)){ 
        $item = array("date" => $date, "new_time" => $new_time[1], 'diff_time' => $diff_time, 'invoice_no' => $row['invoice_no'], 'table_no' => $row['table_type'],'section_no'=>$row['section_type']);
            ...
            <div class="row" id="main-content" style="padding-top:25px">
                <div class="well">
                              ...
                              <tbody>
                                <?php  $i =1;
                                    foreach ($pending_data as $key => $value) {
										 $section_name=$value['section_no'];
										 $section_type = mysqli_fetch_assoc(mysqli_query($conn, "SELECT name FROM sections WHERE id ='$section_name'"));
										?>
                                        <tr style="text-align: center;">
                                            ...
                                            <td style="font-size: 20px;"><?= $section_type['name'];?></td>
                    ...
                    <table class="table table-striped">
                        ...
                        $merchant_name = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id ='".$row['merchant_id']."'"));
                        $section_type = mysqli_fetch_assoc(mysqli_query($conn, "SELECT name FROM sections WHERE id ='".$row['section_type']."'"));
                        $date=date_create($created);
                        ?>
                        <?php
                        ...
                        <tr id="<?php echo $row['invoice_no']; ?>" class="<?php echo $todayorder; ?> fdfd <?php echo $callss; ?>" data-id="<?php echo $row['id']; ?>">
                            ...
                            <td class="table_number_<?php echo $row['id']?>"><?php echo $section_type['name'];?></td>
					...
					<?php } ?>
					<audio id="my_audio" style="display:none;"  autostart="0">
