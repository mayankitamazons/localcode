...
<body class="header-light sidebar-dark sidebar-expand pace-done">
  ...
  <div style="float:left;width:100%">
    ...
    <div style="float:left;width:20%;margin-top:10px;">
        ...
        ?>
			<?php } else { ?>
					    	<h5 class="favorite_name" style="display: inline-blick;margin-left:1%;">We are temporary closed for upgrading our service to serve you better in the future. </h5>
					  <?php } ?>
                    ...
                    <?php 
                      ...
                      foreach ($ingredients as $ingredient) {
                        if(!empty($ingredient)){
                          ...
                          ?>
                           <div style="margin: 10px 8px;" class="btn-group" data-toggle="buttons">
                              ...
                              <?php if(!is_null($ingPrice)){ ?><div class='extra-price-ingredient'><?php echo $ingPrice; ?></div><?php } ?>
                            </div>
                            <?php
                        }
                      }
   ...
   $(".without_varient").on("click", function(){
    ...
    var p_extra = $(this).parent().parent().find("input[name='extra']").val();
      ...
      p_extra = 0;
		...
		var p_total = p_price*quantity+parseFloat(p_extra);
    $("#test").append("<tr>  <td><button type='button' class='removebutton'>X</button> </td><td>"+name+"</td><td><input style='width:50px;'  onchange='UpdateTotal("+id+" ,"+p_price+")'  type=number name='qty[]' maxlength='3'  value="+quantity+" id='"+id+"_test_athy'><input type= hidden name='p_id[]' value= "+id+"><input type= hidden name='p_code[]' value= "+code+"><input type='hidden' name='ingredients' value='" + single_remarks + "'/></td><td>"+code+"</td><td><a href='#remarks_area' role='button' class='introduce-remarks btn btn-large btn-primary' data-toggle='modal'>" + ((single_remarks == '') ? "Remarks" : single_remarks) +  "</a><input type='hidden' name='extra' value='" + p_extra + "'></td><td><input style='width:70px;text-align:right;' type='text' name='p_extra' value='" + p_extra + "' readonly></td><td><input style='width:70px;' type='text' name='p_price[]' value= "+p_price+" readonly></td><td><input type='text' style='width:70px;' name='p_total[]' value= " + p_total + " readonly  id='"+id+"_cat_total'><input type='hidden' name='varient_type[]' value=''></td> </tr>");
     ...
     $("input[name='single_ingredients']").val('');
      ...
      p_extra = 0;
	   ...
	   document.getElementById(child_id).classList.add("fa-plus");
		 ...
		 // alert(varient_type);
	   $("#test").append("<tr>  <td><button type='button' class='removebutton'>X</button> </td><td>"+name+"</td><td><input style='width:50px;'  onchange='UpdateTotal("+id+" ,"+p_price+")'  type=number name='qty[]' maxlength='3'  value="+quantity+" id='"+id+"_test_athy'><input type= hidden name='p_id[]' value= "+id+"><input type= hidden name='p_code[]' value= "+code+"><input type='hidden' name='ingredients' value='" + single_remarks + "'/></td><td>"+code+"</td><td><a href='#remarks_area' role='button' class='introduce-remarks btn btn-large btn-primary' data-toggle='modal'>" + ((single_remarks == '') ? "Remarks" : single_remarks) +  "</a><input type='hidden' name='extra' value='" + p_extra + "'></td><td><input style='width:70px;text-align:right;' type='text' name='p_extra' value='" + p_extra + "' readonly></td><td><input style='width:70px;' type='text' name='p_price[]' value= "+p_price+" readonly></td><td><input type='text' style='width:70px;' name='p_total[]' value= "+ (parseFloat(p_total) + parseFloat(p_extra)) + " readonly  id='"+id+"_cat_total'><input type='hidden' name='varient_type[]' value="+varient_type+"></td> </tr>");
		alert('The product added');
		$("#varient").html('');
	});
	// end product varient 
    ...
    $("#add-ingredient").click(function(){
        ...
        if($("input[name='new-ingredient']").val() == ''){
           alert("The ingredient's input is empty");
            ...
            ingredients.push(complete_ingredient);
            ...
            if($("input[name='price-ingredient']").val() != '' && $("input[name='price-ingredient']").val() != 0){
                $("#ingredients_container").append("<div class='ingredient'><button type='button' class='btn btn-info remove-ingredient' aria-label='Close' ><span aria-hidden='true'>&times;</span></button><span class='ingredient-name'>" + ingredientName + "</span><div class='extra-price-ingredient'>" + $("input[name='price-ingredient']").val() + "</div><input type='hidden' name='ingredient-name-input' value=" + $("input[name='new-ingredient']").val().replace(/ /g,"_") + "[" + $("input[name='price-ingredient']").val() + "]" +  " /></div>");
            }else{
