...
if(isset($_POST['login']))
{
	...
	// die;
 	function updateStatus($session_id,$setup_session,$id){
		...
		setcookie("my_cookie_id", $my_cookie_id, time() + 10 * 365 * 24 * 60 * 60, "/");
		//setcookie("my_token", $session_id, time() + 3600 * 24 * 30 * 12 * 10,"/");
		setcookie("my_token", $session_id, time() + 10 * 365 * 24 * 60 * 60, "/");
	...
	if(empty($error))
	{
		...
		if(!isset($cookie_id) || !isset($session_token)){
				$session_id =  uniqid($id . "_",true);
				setcookie("session_id", $session_id, time() + 3600 * 24 * 30 * 12 * 10,"/");
				setcookie("cookie_user_id", $id, time() + 3600 * 24 * 30 * 12 * 10,"/");
				$_SESSION['login']=$id;
				$_SESSION['user_id']=$id;
				//echo "<pre>"; print_r($_COOKIE);
				//echo "<pre>"; print_r($_SESSION); die;
			if(updateStatus($_COOKIE['PHPSESSID'],$setup_session,$id)){
				//lucky
           ...
           <div class="clear" style="padding:25px;"></div>
                            ...
                            <div class="tab-1 resp-tab-content" aria-labelledby="tab_item-1">
								...
								<div class="login-top sign-top">
									<input type="radio" name="user_role" value="1"> <?php echo $language["member"];?>
