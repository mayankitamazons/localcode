...
$security = mysqli_fetch_assoc(mysqli_query($conn, "SELECT security_questions,security_answer FROM users WHERE id='".$_SESSION['login']."'"));
	$u_id= $order_total['user_id'];
	$m_id= $order_total['merchant_id'];
	$o_id= $order_total['id'];

	$product_ids = explode(",",$order_total['product_id']);
	$quantity_ids = explode(",",$order_total['quantity']);
	...
	$total = 0;
		...
		foreach ($c as $key => $val)
		{
	  	$products = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM products WHERE id ='".$key."'"));	
		   $total =  $total + ($products['product_price'] * $val);
	   }
