...
if(isset($_POST))
{
	...
	else
	{
		if($mobile_number)
		{
			...
			$user_role=1;
			$reocrd=mysqli_query($conn, "INSERT INTO users SET name='$name',user_roles='$user_role',mobile_number='$mobile_check',guest_user='y',login_status='1',password_created='$password_created',otp_verified='$otp_verified'");
	...
	// insert data into order table 
	 ...
	 // die
		...
		if($double_invoice_count==0)
		{
			...
			$section_saved='n';
		  		         $sqlFinalIns = "INSERT INTO order_list SET  status='3',rebate_amount='$rebate_amount',prepaid='$prepaid',total_cart_amount='$total_cart_amount',total_rebate_amount='$total_rebate_amount',wallet_paid_amount='$wallet_paid_amount',online_pay='$online_pay',payment_alert='$payment_alert',user_name='$user_name',user_mobile='$user_mobile',wallet='paypal',varient_type='$v_str',product_id='$pro_id',  user_id='$user_id', merchant_id='$m_id', quantity='$qty_list', amount='$p_price',product_code='$p_code', remark='$option', location='".$location."', table_type='".$table_type."',section_type='$section_type',created_on='$date', invoice_no='$invoice_no',newuser='$newuser',show_alert='$show_alert',section_saved='$section_saved'";
		...
		if($order_completed)
		{
			$array_detail=array('status'=>true,'msg'=>'Order Added,Make payment','order_id'=>$order_id);    
		}
		else
		{
			$array_detail=array('status'=>false,'msg'=>'Failed to place order Try Again');
		}
